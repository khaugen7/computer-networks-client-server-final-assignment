...
def main():
    ...
    # If we receive a redirect response then we want to attempt to retrieve the file from the new server

    if response_list[1] == '301':
        print("Redirect response received from server. Details:\n")
        print(response_line)
        location = ''
        bytes_to_read = 0
        while not headers_done:
        ...
        print_file_from_socket(client_socket, bytes_to_read)
        print('Attempting to retrieve file from redirect: %s' % location)
        sys.argv[1] = location
        main()
